...
> **Status**: early development. The core matching engine (config parsing, path matching, `enter`/`leave` execution) and the bash/zsh shell hooks exist and are tested, but matched blocks aren't executed yet — that needs the `envoke allow` trust mechanism, which doesn't exist yet either. This README describes the target shape of v1. See [CLAUDE.md](CLAUDE.md) for the MVP scope order.
